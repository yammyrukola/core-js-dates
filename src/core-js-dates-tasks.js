...
function getNextFriday(date) {
  const k = 1000 * 60 * 60 * 24;
  const currentDay = date.getDay();
  const delta = currentDay < 5 ? 5 - currentDay : 5 + (7 - currentDay);

  return new Date(date.getTime() + k * delta);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const delta = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  if (delta === 0) return 0;

  return Math.floor(delta / (24 * 3600 * 1000)) + 1;
}
...
function formatDate(dateVal) {
  const [year, month, day, hour, minute, second] = dateVal.match(/\d+/g);
  ...
  const prefix = h < 12 ? 'AM' : 'PM';
  const hours = h % 12 === 0 ? 12 : h % 12;

  const res = `${m}/${d}/${YYYY}, ${hours}:${minutes}:${s} ${prefix}`;
  return res;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= endDate; i += 1) {
    ...
    counter = day === 0 || day === 6 ? counter + 1 : counter;
  }
...
function getWeekNumberByDate(date) {
  ...
  const deltaMs = date - firstOfYear;
  const deltaDays = deltaMs / (24 * 3600 * 1000);

  const add = firstOfYear.getDay() === 0 ? 6 : firstOfYear.getDay() - 1;
  const week = Math.ceil((deltaDays + add) / 7);
